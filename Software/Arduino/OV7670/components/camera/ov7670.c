...
{
  SCCB_Write(sensor->slv_addr, OV7670_CLKRC_REG, frame_rate_cfg->clkrc);
  systick_sleep(1);
  SCCB_Write(sensor->slv_addr, OV7670_DBLV_REG, frame_rate_cfg->dblv);
  systick_sleep(1);
  SCCB_Write(sensor->slv_addr, OV7670_EXHCH_REG, frame_rate_cfg->exhch);
  systick_sleep(1);
  SCCB_Write(sensor->slv_addr, OV7670_EXHCL_REG, frame_rate_cfg->exhcl);
  systick_sleep(1);
  SCCB_Write(sensor->slv_addr, OV7670_DM_LNL_REG, frame_rate_cfg->dm_lnl);
  systick_sleep(1);
  SCCB_Write(sensor->slv_addr, OV7670_DM_LNL_REG, frame_rate_cfg->dm_lnh);
  systick_sleep(1);
  // store new clkrc
...
static void OV7670_LightMode(sensor_t *sensor, ov7670_light_mode_cfg_t *light_mode_cfg)
{
  ...
  SCCB_Write(sensor->slv_addr, OV7670_GAIN_REG, 0x00);
  systick_sleep(1);

  // Exposure value
  SCCB_Write(sensor->slv_addr, OV7670_AECH_REG, 0x00);
  systick_sleep(1);
  SCCB_Write(sensor->slv_addr, OV7670_AECHH_REG, 0x00);
  systick_sleep(1);
  I2CSet(sensor->slv_addr, OV7670_COM1_REG, 0x3, 0x00);
  systick_sleep(1);
  // AGC/AEC stable operation region configuration
  SCCB_Write(sensor->slv_addr, OV7670_AEW_REG, 0x75);
  systick_sleep(1);
  SCCB_Write(sensor->slv_addr, OV7670_AEB_REG, 0x63);
  systick_sleep(1);
  SCCB_Write(sensor->slv_addr, OV7670_VPT_REG, 0xd4);
  systick_sleep(1);
}
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
  ...
  case PIXFORMAT_RGB565:
    ...
    /* for (i = 0; rgb565_ov7670[i][0]; i++)
    {
      SCCB_Write(sensor->slv_addr, rgb565_ov7670[i][0], rgb565_ov7670[i][1]);
    }*/
...
static int set_framerate(sensor_t *sensor, framerate_t framerate)
{
  ...
  if (framerate == 0)
    OV7670_FrameRateAdjustment(sensor, &OV7670_14FPS_12MHZ_XCLK);
  if (framerate == 1)
    OV7670_FrameRateAdjustment(sensor, &OV7670_15FPS_12MHZ_XCLK);
  if (framerate == 2)
    OV7670_FrameRateAdjustment(sensor, &OV7670_25FPS_12MHZ_XCLK);
  if (framerate == 3)
    OV7670_FrameRateAdjustment(sensor, &OV7670_30FPS_12MHZ_XCLK);
